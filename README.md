...
- Correct action on the right column → positive reward (+0.05 to +0.25)
- Useless action (already clean column) → 0.00
- Destructive action (e.g. drop_nulls on 20% of rows) → negative reward (−0.05 to −0.60)
- Unknown action / missing column → −0.05 penalty
...
- 10% of `salary` values contain `" USD"` suffix (mixed types)
...
Click **POST /step** and send an action. Examples:
...
> **Note:** The web UI `params` field requires a proper JSON object, not a string. If you see a validation error, ensure you're entering JSON directly (no surrounding quotes).
 
### 3. Check current state
 
Click **GET /state** — returns `episode_id`, `step_count`, `task_id`, `current_f1`, `is_terminated`.
...
Below is an actual run of the `Qwen/Qwen2.5-72B-Instruct` agent on this environment.

### Why Task 2 shows `0.00` rewards:
In the log below, you will notice the agent occasionally receives a `reward=0.00` (Step 1 & 2). This is a **feature, not a bug.** The DataGym grader is strict: even if the agent cleans the text values in a column, it receives no reward until it **corrects the data type** (Step 3). This prevents "half-fixes" and ensures the agent learns to complete the entire pipeline.
...
└── server/
    ├── app.py            # FastAPI app created via create_app()
...
**Grader design:** Cell-level F1 with separate precision/recall denominators — extra duplicate rows lower precision (rewarding dedup), dropped rows lower recall (penalising drop_nulls). DateTime columns compared date-only after normalisation so `cast_type(datetime)` gives real signal.
 
**Issues-as-instructions:** `detect_issues()` runs after every action and emits concrete fix instructions with exact mappings (e.g. `{"999999.99": "260.41"}`) rather than vague warnings. The agent never has to guess.
...
### 3. HF Web "UI Pro-Tip" (To prevent user error)
Since the Hugging Face Web UI can be finicky with data types, add a small warning in the **"Execute a step"** section.

**Add this right after the "Execute a step" examples:**
> **⚠️ UI CRITICAL:** When using the Web UI buttons, the **Params** field must be a valid JSON object (e.g., `{"strategy": "mode"}`). If you enter it as a string or leave it blank when an action requires it, the Pydantic validator will reject the request with a `422 Unprocessable Entity` error to protect the environment state.
